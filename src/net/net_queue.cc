...
int CNetQueue::DeQueue(char *buf, uint32_t buf_size, uint32_t *data_len)
{
    ...
    *data_len = real_len;
    ...
    char c;
    if (read(m_fd[0], &c, 1) == 1) {
        m_read_num++;
    } else {
        printf("read faile![%d][%d][%s]\n", errno, EAGAIN, strerror(errno));
    }
    return 0;
}
...
{
    char buf[10240];
    snprintf(buf, 10240, "push_num=%d,pop_num=%d,size=%d,write_num=%d,reade_num=%d,head[%d],tail[%d]",
             m_enqueue_num, m_dequeue_num, m_enqueue_num - m_dequeue_num, m_write_num, m_read_num,
             m_head, m_tail);
    return buf;
}
