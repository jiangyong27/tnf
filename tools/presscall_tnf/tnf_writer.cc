...
uint32_t CTNFWriter::GetSeq(char *data_buf, uint32_t data_len)
{
...
void CTNFWriter::OnSend(writer_data_t *data)
{
    ...
    if (data->send_pos >= data->send_len) {
        //LOG_ERROR(m_logger, "send net seq[%d]", GetSeq(data->send_buf, data->send_len));
        data->send_len = 0;
        data->send_pos = 0;
        m_epoll->Mod(data->fd, data, true, false);
    }
...
void CTNFWriter::OnRecv(writer_data_t* data)
{
    ...
    if (ret > 0) {
        //LOG_ERROR(m_logger, "recv net seq[%d]", GetSeq(data->recv_buf, ret));
        if (ret < (int)data->recv_len) {
            ...
            uint32_t pos = ret;
            while (1) {
                ret = net_complete_longconn(data->recv_buf + pos, data->recv_len - pos);
                if (ret <=0 ){
                    break;
                }
                memcpy(m_buffer + sizeof(writer_rsp_header_t), data->recv_buf + pos, ret);
                if (0 != m_rsp_queue->Push(m_buffer, ret + sizeof(writer_rsp_header_t))) {
                    LOG_ERROR(m_logger, "recv queue push failed!");
                }
                pos += ret;
            }
            memcpy(data->recv_buf, data->recv_buf + pos , data->recv_len - pos);
            data->recv_len = data->recv_len - pos;
